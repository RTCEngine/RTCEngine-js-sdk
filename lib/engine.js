...
class RTCEngine extends events.EventEmitter
{
    ...
    addStream(stream)
    {
        ...
        .then(() => {
            ...
            if(stream._hasMedia){
                // we better use old api for now  
                logger.debug("real add stream");
    ...
    _createPeerConnection()
    {
        ...
        logger.debug('peerconnection', options);

        this._peerconnection = new RTCPeerConnection(options);

        this._peerconnection.addEventListener('iceconnectionstatechange',(state) => 
        {
        
            logger.debug('iceconnectionstatechange ===== ', state ,this._peerconnection.iceConnectionState);
        ...
        this._peerconnection.addEventListener('negotiationneeded', (state) => {
            logger.debug('peerconnection "negotiationneeded" event ', state);
        });
    ...
    _renegotiation()
    {
        ...
        logger.debug('renegotiation iceConnectionState ',  this._peerconnection.iceConnectionState);
