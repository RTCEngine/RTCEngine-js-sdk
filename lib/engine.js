...
class RTCEngine extends events.EventEmitter
{
    ...
    addStream(stream)
    {
        ...
        .then(() => {
            ...
            if(stream._audioTrack) {
                stream._audioSender = this._peerconnection.addTrack(stream._audioTrack,stream._stream)
            }

            if(stream._videoTrack) {
                stream._videoSender = this._peerconnection.addTrack(stream._videoTrack,stream._stream)
            }
    ...
    _removeStream(stream)
    {
        ...
        .then((offer) => {
            ...
            let answer = this._peerconnection.remoteDescription
