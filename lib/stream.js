...
export default class RTCStream extends events.EventEmitter
{
    constructor(options)
    {
        ...
        this._desktopStreamId = options.desktopStreamId || null;
    ...
    setAttributes(data){
        if(!this._local){
            throw 'only local stream can change attributes'
        }
            ...
            .then((unlock) => {
                ...
                return this._getLocalStream(constraints)
                    ...
                    .catch((error) => {
                        ...
                        logger.debug('getUserMedia error', error);
                        throw error;
                    });
    ...
    _updateAttributes(){
        if(this._engine){
            ...
            this._engine._updateAttributes(data)
        }
    }
    _setAttributes(data){
        if(!this._local){
